...
import { fetchAdminDashboardData } from "@/features/admin/services/admin-dashboard-data";
import { fetchAdminOrderEntryData } from "@/features/admin/services/admin-order-entry-data";
import AdminOrderEntryPanel from "@/features/admin/components/AdminOrderEntryPanel";
...
function getAdminDisplayName(adminUsername: string) {
  switch (adminUsername) {
    case "085601679005":
      return "Ardyan Prasetya";
    case "087728386861":
      return "Rochmad Agung";
    default:
...
export default async function AdminDashboardPage({
  ...
  const [data, orderEntryData] = await Promise.all([
    fetchAdminDashboardData(),
    fetchAdminOrderEntryData(),
  ]);
  const adminDisplayName = getAdminDisplayName(adminUsername);
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white">
      <section className="border-b border-white/10 bg-zinc-950/90">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-8 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            <span className="inline-flex items-center rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-2 text-sm font-medium text-emerald-200">
              Login sebagai {adminDisplayName}
            </span>
            <Link
              href="/"
      ...
      <section className="mx-auto max-w-7xl space-y-8 px-6 py-8">
        <AdminOrderEntryPanel data={orderEntryData} />
        ...
        <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-5 text-sm leading-relaxed text-emerald-100">
          <p className="font-semibold text-emerald-300">Status keamanan</p>
          <p className="mt-2 text-emerald-100/80">
            Dashboard ini sudah memakai login admin berbasis cookie `httpOnly`.
            Pastikan credential admin dan secret session diatur aman pada environment.
          </p>
        </div>
      </section>
    </main>
